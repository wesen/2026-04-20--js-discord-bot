...
func buildDiscordOps(scriptPath string, session *discordgo.Session) *DiscordOps {
	...
	return &DiscordOps{
		...
		MessagePin: func(ctx context.Context, channelID, messageID string) error {
			_ = ctx
			channelID = strings.TrimSpace(channelID)
			messageID = strings.TrimSpace(messageID)
			if channelID == "" || messageID == "" {
		...
		MessageUnpin: func(ctx context.Context, channelID, messageID string) error {
			_ = ctx
			channelID = strings.TrimSpace(channelID)
			messageID = strings.TrimSpace(messageID)
			if channelID == "" || messageID == "" {
		...
		MessageListPinned: func(ctx context.Context, channelID string) ([]map[string]any, error) {
			_ = ctx
			channelID = strings.TrimSpace(channelID)
			if channelID == "" {
