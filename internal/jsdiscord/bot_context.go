...
func buildDispatchInput(vm *goja.Runtime, ctx context.Context, request DispatchRequest) *goja.Object {
	...
	setObjectField(vm, input, "interaction", request.Interaction.ToMap())
	setObjectField(vm, input, "message", request.Message.ToMap())
	setObjectField(vm, input, "before", request.Before)
	setObjectField(vm, input, "user", request.User.ToMap())
	setObjectField(vm, input, "guild", request.Guild)
	setObjectField(vm, input, "channel", request.Channel)
	setObjectField(vm, input, "member", request.Member.ToMap())
	setObjectField(vm, input, "reaction", request.Reaction.ToMap())
	setObjectField(vm, input, "me", request.Me.ToMap())
	setObjectField(vm, input, "metadata", request.Metadata)
	setObjectField(vm, input, "config", request.Config)
	setObjectField(vm, input, "component", request.Component.ToMap())
	setObjectField(vm, input, "modal", request.Modal)
	setObjectField(vm, input, "focused", request.Focused.ToMap())
